	    ...
	    (0, _directiveDef.def)(name, {
	        ...
	        link: function link($scope, element, attrs) {
	            ...
	            (0, _baiduScriptLoader.loader)($scope.ak, offlineOpts, function () {
	                ...
	                var watch = $scope.$watch('options.markers', function (newValue, oldValue) {
	                    (0, _map.redrawMarkers)(map, previousMarkers, opts);

	                    if(newValue !== oldValue){

                            var start = oldValue.length;
                            newValue = newValue.slice(start);
                            
                            if(newValue[0]['circle']){
                            	remove_overlay();
                                creatCircle(newValue); 
                            }
	                    }	                      

	                }, true);

	                function creatCircle (marker) {
...
/***/ function(module, exports) {
	...
	var loader = exports.loader = function loader(ak, offlineOpts, callback) {
	    var MAP_URL = 'js/common/apiv2.0.min.js';
	    ...
	    window.BMap.apiLoad = function() {
	        ...
	        if (typeof baidumapinit == "function") {
	            //baidumapinit();
	            setTimeout(function(){baidumapinit();}, 1000);
	        }
	    };
...
/***/ function(module, exports) {
	...
	var createInstance = exports.createInstance = function createInstance(opts, element) {
	    // create map instance
	    var map = new BMap.Map("deviceViewMap", {mapType: BMAP_NORMAL_MAP});
	    ...
	    opts.markers.forEach(function (marker) {
	        ...
	        previousMarker.listener = function () {
	            ...
	            if(!!opts.onMarkerSelect){
	            	opts.onMarkerSelect(marker);
	            }
	            $('.deviceInfoBox').show();
	            $('.pointName').html(marker.title);
	            $('.pointNumber').html(marker.number);
	            $('.pointContent').html(marker.content);
	            $('.pointLat').html(marker.latitude);
	            $('.pointLon').html(marker.longitude);
	            $('.pointDesc').html(marker.desc);
	            $('.pointType').html(marker.type);
	            $('.numberNetCart').html(marker.numberNetCart);
	        };
